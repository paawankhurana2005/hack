// Return-pricing route — GET /api/return-pricing/:returnId returns the live price
// breakdown for a DB-backed return record. Pure read: calls the write-free pricing
// engine, so it's safe to hit on every page view. Mirrors the DB-backed route
// convention used by state.ts / auth.ts (mongo guard → 503, apiError helper).
// NOTE: renamed from /api/pricing at the mongodb-branch merge — /api/pricing is
// the spec-014 event-driven reprice engine (decide/outcome/state).
...
export function createReturnPricingRouter(): Router {
  const router = Router();
