...
// Exposed as a pure sync function so the real VLM comparator can use it as its
// deterministic fallback (same ReferenceComparator contract).
...
/** Pure, deterministic reference comparison (no model call). */
export function mockComparison({ grade, detectedIssues, reference }: ReferenceInput): ReferenceComparison {
