    ...
    <Card>
      <div className="animate-pulse space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="h-6 w-12 rounded bg-navy-700" />
        </div>
        <div className="h-2 w-full rounded bg-navy-700" />
        <div className="space-y-2">
          <div className="h-3 w-3/4 rounded bg-navy-700" />
          <div className="h-3 w-1/2 rounded bg-navy-700" />
        </div>
        <div className="mt-4 text-center">
          <p className="text-sm font-semibold text-white">Inspecting your item…</p>
          <p className="mt-1 text-xs text-muted">AI is reviewing your photos</p>
        </div>
      </div>
    </Card>
  );
...
export function Step2Grading({ flowState, onNext, gradingScenario }: Props) {
  ...
  if (status === 'warehouse_fallback') {
    return (
      <Card>
        <p className="font-semibold text-white">Grading inconclusive</p>
  ...
  if (status === 'retake') {
    return (
      <Card>
        <p className="font-semibold text-white">Clearer photo needed</p>
        <p className="mt-2 text-sm text-muted">
        ...
        <div className="mt-4 flex gap-3">
          ...
          <button
            type="button"
            onClick={handleBridgeFallback}
            className="rounded-md border border-navy-600 px-5 py-2.5 text-sm text-muted hover:border-navy-500"
          >
        ...
        {retryPhotos.length > 0 && (
          <div className="mt-4 flex gap-2">
            {retryPhotos.map((url) => (
              ...
              <img key={url} src={url} alt="retry" className="h-16 w-16 rounded-md object-cover" />
            ))}
          </div>
        )}
      </Card>
    ...
    <Card>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          {result.grade && (
            <span
              className={`rounded border px-3 py-1 text-sm font-bold ${GRADE_COLORS[result.grade] ?? 'bg-navy-700 text-white'}`}
            >
              Grade {result.grade}
            </span>
          )}
