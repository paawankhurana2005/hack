...
export function BuyerStep2Pickup({
  ...
  // Routing still runs and is persisted (the seller dashboard reads it) — the
  // buyer just isn't shown the decision, so only the setter is read here.
  const [, setRouting] = useState<ReturnRoutingDecision | null>(null);
  // Still minted and persisted onto the return record — just no longer shown to
  // the buyer, so only the setter is read here.
